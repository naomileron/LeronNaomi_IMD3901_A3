...
public class PlayerInteraction : NetworkBehaviour
{
    ...
    private void Update()
    {
        ...
        //Press E to lower bed (only if aiming at button)
        if (Keyboard.current.eKey.wasPressedThisFrame && targeting.IsAimingAtBedButton())
        {
            //Debug.Log("[PlayerInteraction] E pressed while aiming at button");
            buttonClick.Play();
            targeting.CurrentPatient.LowerBedServerRpc();
            bedLower.Play();
        }

        //Press Space to do one compression (only if aiming at patient's chest)
